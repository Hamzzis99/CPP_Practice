// TableDescripotrHeap.cpp
#include "pch.h"
#include "TableDescripotrHeap.h"
#include "Engine.h"

void TableDescriptorHeap::Init(uint32 count) // 몇 그룹으로 만들어줄지.
{
	_groupCount = count;

	D3D12_DESCRIPTOR_HEAP_DESC desc = {};
	desc.NumDescriptors = count * REGISTER_COUNT;
	desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE; // 얘를 지켜줘야 GPU의 메모리들을 위로 올려준다.
...
void TableDescriptorHeap::CommitTable()
{
	...
	handle.ptr += _currentGroupIndex * _groupSize; // 현재 있는 그룹테이블을 위로 올린다.
